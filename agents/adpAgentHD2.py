...
class AdpAgentHD2():
    def __init__(self, env: SimplePlant, settings: dict):
        super(AdpAgentHD2, self).__init__()
    ...
    def learn(self, epochs = 10):
        ...
        for i in tqdm(range(epochs)):
            ...
            for t in range(self.env.T):
                ...
                logging.info(f"\t {actual} + {self.discount} * {tot_future}")
                logging.info(f"\t {pd_data} \ {actual + self.discount * tot_future}")
                self.post_decision_value_function.fit(
                    pd_data, actual + self.discount * tot_future
                )
                logging.info(f"\t \t V[0] {self.post_decision_value_function.V[0]}")
                logging.info(f"\t \t V[1] {self.post_decision_value_function.V[1]}")
