...
class SimplePlantSB1(SimplePlant):
    def __init__(self, settings, stoch_model, action_position,base_rl_agent):
        ...
        self.next_action =  np.array(settings['initial_setup'])
    ...
    def step(self, action):
        ...
        self.base_agent_action = self.base_rl_agent.get_action(self.obs_sb)
        
        try:
            if self.base_agent_action == None:
                # print("It is None")
                # action = self.base_rl_agent.get_action(self.obs_sb)
                self.base_agent_action = self.machine_setup
        except:
            pass
        if action == 1:  
            self.base_agent_action[self.action_position] = self.base_agent_action[self.action_position]
        else:
            self.base_agent_action[self.action_position] = self.machine_setup[self.action_position]
        
        self.total_cost = self._take_action(self.base_agent_action, self.machine_setup, self.inventory_level, self.demand)
        ...
        self.last_action = self.base_agent_action
        
        obs = self._next_observation()
        
        try:
            if self.base_rl_agent.get_action(self.obs_sb) == None:
                # print("It is None")
                # action = self.base_rl_agent.get_action(self.obs_sb)
                self.next_action = self.machine_setup
        except:
            pass
         
        self.next_action = self.base_rl_agent.get_action(self.obs_sb)
...
class MultiAgentRL():
    ...
    def get_action(self, obs):
        ...
        try:
            if base_agent_action == None:
                # print("It is None")
                # action = self.base_rl_agent.get_action(self.obs_sb)
                base_agent_action = obs['machine_setup']
        except:
            pass
        ...
        for i in range(self.settings['n_machines']):
            ...
            if action == 1:  
                base_agent_action[i] = base_agent_action[i]
            else:
                base_agent_action[i] = obs['machine_setup'][i]
        
        
        # Here we use the other agents impact to make the actual decision:
        if self.dict_obs:
            obs['next_action'] = copy.copy(list(base_agent_action))
        else:
            obs = np.concatenate((np.array(obs['inventory_level']), # n_items size
