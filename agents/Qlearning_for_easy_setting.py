...
class Qlearning_for_easy_setting():
    def __init__(self, env: SimplePlant, settings: dict):
        super(Qlearning_for_easy_setting, self).__init__()
        self.env = copy.deepcopy(env)
        self.env_for_exp = copy.deepcopy(env)
        self.discount = settings['discount_rate']
        self.post_decision_value_function = RegressorLib('plain_matrix_I2xM1')
    ...
    def train_policy(self):
        n_scenarios = self.env.scenario_demand.shape[1]
        max_demand = np.max(self.env.scenario_demand, axis=1)
        ...
        for i in range(self.env.n_items):
            ...
            for ele in self.env.scenario_demand[i,:]:
    ...
    def _compute_best_action(self, obs, epsilon_prob=0):
        if self.env.n_machines > 2:
            # small neiborhood search start form the solution of the rule
